    ...
    public class WaveFormRenderer : RendererBase
    {
        ...
        protected override void OnConfigurationChanged()
        {
            if (this.Configuration != null)
            {
                this.Mode = this.Configuration.GetElement<SelectionConfigurationElement>(
                    ...
                    WaveFormStreamPositionConfiguration.MODE
                );
                ...
                this.Rms = this.Configuration.GetElement<BooleanConfigurationElement>(
                    WaveFormStreamPositionConfiguration.SECTION,
                    WaveFormStreamPositionConfiguration.RMS
                );
                this.Logarithmic = this.Configuration.GetElement<BooleanConfigurationElement>(
                    WaveFormStreamPositionConfiguration.SECTION,
                    WaveFormStreamPositionConfiguration.DB
                );
                this.Smoothing = this.Configuration.GetElement<IntegerConfigurationElement>(
                    WaveFormStreamPositionConfiguration.SECTION,
                    WaveFormStreamPositionConfiguration.SMOOTHING
                );
                this.ColorPalette = this.Configuration.GetElement<TextConfigurationElement>(
                    WaveFormStreamPositionConfiguration.SECTION,
                    WaveFormStreamPositionConfiguration.COLOR_PALETTE
                );
        ...
        protected override void ClearRendererTarget(RendererTarget target)
        {
            ...
            try
            {
                ...
                if (data != null)
                {
                    info = BitmapHelper.CreateRenderInfo(target, data.Colors[WaveFormStreamPositionConfiguration.COLOR_PALETTE_BACKGROUND]);
                }
                else
                {
                    ...
                    info = BitmapHelper.CreateRenderInfo(target, palettes[WaveFormStreamPositionConfiguration.COLOR_PALETTE_BACKGROUND]);
                }
        ...
        private static WaveFormRenderInfo GetRenderInfo(RendererTarget target, WaveFormRendererData data)
        {
            ...
            {
                Background = BitmapHelper.CreateRenderInfo(target, data.Colors[WaveFormStreamPositionConfiguration.COLOR_PALETTE_BACKGROUND])
            };
            if (data.PowerElements != null)
            {
                info.Rms = BitmapHelper.CreateRenderInfo(target, data.Colors[WaveFormStreamPositionConfiguration.COLOR_PALETTE_RMS]);
            }
            if (data.WaveElements != null)
            {
                info.Value = BitmapHelper.CreateRenderInfo(target, data.Colors[WaveFormStreamPositionConfiguration.COLOR_PALETTE_VALUE]);
            }
            return info;
        }
