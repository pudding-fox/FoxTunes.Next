...
namespace FoxTunes
{
    public static class BassResamplerStreamComponentConfiguration
    {
        ...
        public const string ENABLED = "AAAA5C85-178C-470D-A977-C54350875AB3";

        public const string QUALITY = "BBBB0ED2-67E6-4155-AFB0-FE8D7E7B9B8C";

        public const string PHASE = "CCCC4941-E1DC-4C2E-9DF7-14C1638C392F";

        public const string STEEP_FILTER = "DDDD0064-06D7-4EFA-B3C9-2820B8893039";

        public const string INPUT_BUFFER_LENGTH = "EEFF4B8B1A-4B97-4971-AA4D-40ACC2C68828";

        public const string PLAYBACK_BUFFER_LENGTH = "FFFF63F8-2695-4853-89E8-ADC10D8EFC2E";

        public static IEnumerable<ConfigurationSection> GetConfigurationSections()
        {
            yield return new ConfigurationSection(SECTION)
                .WithElement(new BooleanConfigurationElement(ENABLED, Strings.BassResamplerStreamComponentConfiguration_Enabled, path: Strings.BassResamplerStreamComponentConfiguration_Path).WithValue(false))
                .WithElement(new SelectionConfigurationElement(QUALITY, Strings.BassResamplerStreamComponentConfiguration_Quality, path: Strings.BassResamplerStreamComponentConfiguration_Path).WithOptions(GetQualityOptions()).DependsOn(BassOutputConfiguration.SECTION, ENABLED))
                .WithElement(new SelectionConfigurationElement(PHASE, Strings.BassResamplerStreamComponentConfiguration_Phase, path: Strings.BassResamplerStreamComponentConfiguration_Path).WithOptions(GetPhaseOptions()).DependsOn(BassOutputConfiguration.SECTION, ENABLED))
                .WithElement(new BooleanConfigurationElement(STEEP_FILTER, Strings.BassResamplerStreamComponentConfiguration_SteepFilter, path: Strings.BassResamplerStreamComponentConfiguration_Path).WithValue(false).DependsOn(BassOutputConfiguration.SECTION, ENABLED))
                ...
                .WithElement(new IntegerConfigurationElement(PLAYBACK_BUFFER_LENGTH, Strings.BassResamplerStreamComponentConfiguration_PlaybackBufferLength, path: Strings.BassResamplerStreamComponentConfiguration_Path).WithValue(300).WithValidationRule(new IntegerValidationRule(100, 10000)).DependsOn(BassOutputConfiguration.SECTION, ENABLED)
            );
        }
