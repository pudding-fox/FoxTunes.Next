...
namespace FoxTunes
{
    public static class BassEncoderBehaviourConfiguration
    {
        ...
        public const string ENABLED = "AAAA1152-CA98-4FE5-B63B-B6DD90E11B88";

        public const string DESTINATION = "BBBBBF10-E515-4A99-B49B-4C5821A22945";
        ...
        public const string DESTINATION_LOCATION = "CCCC1639-B91A-44A1-8579-364639C3F52C";

        public const string COPY_TAGS = "DDDD37B0-7CB2-4E55-9DD4-8B1F7DEF0EC9";

        public const string THREADS = "EEEE8113-0C57-464D-B370-C3B77AC048D2";

        public static IEnumerable<ConfigurationSection> GetConfigurationSections()
        {
            yield return new ConfigurationSection(SECTION, Strings.General_Converter)
                .WithElement(
                    new BooleanConfigurationElement(ENABLED, Strings.BassEncoderBehaviourConfiguration_Enabled)
                        .WithValue(false))
                .WithElement(
                    new SelectionConfigurationElement(DESTINATION, Strings.BassEncoderBehaviourConfiguration_Destination)
                        .WithOptions(GetDestinationOptions())
                        .DependsOn(SECTION, ENABLED))
                .WithElement(
                    new TextConfigurationElement(DESTINATION_LOCATION, Strings.BassEncoderBehaviourConfiguration_Location)
                        .WithValue(Path.Combine(Publication.StoragePath, Strings.General_Converter))
                        .WithFlags(ConfigurationElementFlags.FolderName)
                        .DependsOn(SECTION, ENABLED)
                        .DependsOn(SECTION, DESTINATION, DESTINATION_SPECIFIC_OPTION))
                .WithElement(
                    new BooleanConfigurationElement(COPY_TAGS, Strings.BassEncoderBehaviourConfiguration_CopyTags)
                    .WithValue(true)
                    .DependsOn(SECTION, ENABLED))
                .WithElement(
                    new IntegerConfigurationElement(THREADS, Strings.BassEncoderBehaviourConfiguration_Threads)
